...
const SolveWithAI: React.FC<any> = () => {
  ...
  const uploadPhotoInFirebase = async () => {
    ...
    try {
      await fetch('https://firebasestorage.googleapis.com/v0/b/' + storageUrl + '/o?name=' + fileName, {
        method: 'POST',
        headers: {
          'Content-Type': 'image/jpeg' || 'image/png' || 'image/jpg',
        },
        body: await fetch(imageUri as string).then((response) => response.blob()),
      });
      FlaskAPICall();
    } catch (error) {
  ...
  return (
    <View style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fafafa', height: '100%' }}>
      <View style={styles.imageUploadButtonContainer}>
        ...
        <Image style={styles.imageStyle} height={'auto'} width={'auto'} source={{ uri: imageUri as any }} />
      </View>
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.gridContainer}>
              {grid == `Solution doesn't exist. Model misread digits.` ? <Text>{grid}</Text> : grid.map((row: any, rowIndex: number) => (
                <View key={rowIndex} style={styles.rowContainer}>
                  {row.map((value: any, columnIndex: number) => (
