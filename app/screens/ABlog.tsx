...
import { View, Text, Image, TouchableOpacity, TextInput, Dimensions, ScrollView, ActivityIndicator, StyleSheet } from 'react-native';
...
export default function ABlog({ route }: any) {
  const editorRef = useRef<any>(null);
  let params: any = {};
  try {
    // dynamically require to avoid TS type mismatch with expo-router versions
    const xr = require('expo-router');
    params = xr.useSearchParams ? xr.useSearchParams() : {};
  } catch (e) {
    params = {};
  }

  const blogRefParam = (route && route.params && route.params.blogRef) || params?.blogRef;
  const blogDocRef = doc(db, 'blogs', blogRefParam as string);
  ...
  const fetchBlogData = async () => {
    try {
      const blogDoc = await getDoc(doc(db, 'blogs', blogRefParam as string));
  ...
  const getImageUrlToShow = (image: string) => {
  ...
  const newLikeOrRemoveLike = () => {
    if (likes.includes(userRef) == true) {
      removeLike();
    } else {
      addLike();
      removeDislike();
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.actions}>
        <TouchableOpacity onPress={newLikeOrRemoveLike} style={[styles.likeDislikeCommentBtn]}>
          {likes.includes(userRef) == true ? <AntDesign name="like" size={20} color="blue" /> : <Feather name="thumbs-up" size={20} color="green" />}
        ...
        <TouchableOpacity onPress={newDislikeOrRemoveDislike} style={[styles.likeDislikeCommentBtn]}>
          {dislikes.includes(userRef) == true ? <AntDesign name="dislike" size={20} color="#db190b" /> : <Feather name="thumbs-down" size={20} color="red" />}
